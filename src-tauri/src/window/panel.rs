...
/// M1 ships the simple "anchor to cursor + clamp" version; M6 will replace
/// this with edge-flip-aware placement.
pub fn compute_position(
    cursor: (i32, i32),
    monitor_size: (i32, i32),
    panel_size: (i32, i32),
) -> PhysicalPosition<i32> {
    let raw = (cursor.0 + CURSOR_OFFSET_X, cursor.1 + CURSOR_OFFSET_Y);
    let (x, y) = clamp_to_monitor(raw, monitor_size, panel_size);
...
mod tests {
    ...
    #[test]
    fn compute_offsets_from_cursor() {
        let p = compute_position((100, 100), MON, PANEL);
        assert_eq!((p.x, p.y), (100 + CURSOR_OFFSET_X, 100 + CURSOR_OFFSET_Y));
    }

    #[test]
    fn compute_clamps_past_screen_edge() {
        let p = compute_position((1900, 1070), MON, PANEL);
        assert_eq!((p.x, p.y), (1540, 840));
    }
}
